...
window[GRN_LHH].main([window,window.document,jQuery],
	function(window,document,$,undefined){
		...
		EditTables.prototype = {
			...
			'createTextBox':function(element, value) {
				...
				if (editState != "true") {
					...
					//改变状态变量
					$element.attr('EditState','true').parent().parent().attr('CurrentRow',element.parentNode.rowIndex);
			...
			'createDropDownList':function(element, value) {
				...
				if (editState != "true") {
					//添加列表项
					var items = $element.attr("DataItems");
					if (!items) {
					...
					if (items) {
						...
						System.each(items,function(){
							var option=System.Html.tag('option',{'type':'text','text':this.text,'value':this.value});
							options.push(option);
					...
					//记录状态的改变
					$element.attr('EditState','true').parent().parent().attr('LastEditRow',element.parentNode.rowIndex);
		...
		System.extends(EditTables,System.Dom,1);
