...
(function(global,namespace,factory,undefined){
	...
	if(!Array.prototype.each){
		Array.method('each',function(fn){
			if(!isFunction(fn)){
				return this;
			}
			for(var i=0,len=this.length;i<len;++i) {
				if(this[i] && fn.call(this[i], i, this)){
					break;
				}
			}
	...
	if(!Array.prototype.merge){
		Array.method('merge',function(arr,override){
			...
			if(!isArray(arr)){
				throw new Error('Warning '+arr+' 不是数组');
				return;
			}
	...
	MySystem={
		 ...
		 */
		'use':function(){
			this.Config.render.create=true;
			this.Config.render.script.Attribute.async=true;
			return this;
		},
		/**
		 * @author: lhh
		 ...
		 */
		'unUse':function(){
			this.Config.render.create=false;
			this.Config.render.script.Attribute.async=false;
			return this;
		},
		...
		'isClassFile':function(path) {
			...
			if(path.search(/.class/ != -1)){
				...
				if(path.indexOf(".") != -1){
					...
					className=arr[0];
					//这个类文件已经加载过了
					if(isFunction(MySystem[className])){
						return true;
					}
				}
			}
		...
		'is':function(System,useClassName,className){
			if(!(useClassName in System)){
				throw new Error("Warning "+System+" is not a legitimate object or '"+useClassName+"' is not a legitimate");
				return false;
			}
			className = className || null;
			if(!isFunction (System[useClassName])){
				throw new Error("Warning cannot find the class file '/"+useClassName+".class'");
				return false;
			}
			if(!empty(className) && isFunction (System[className])) {
				throw new Error("Warning Class name '"+className+"' already exists");
				return false;
			}
		...
		'defined':function(System,name,M){
			...
			if(!empty(name) && System[name]) {
				throw new Error("Warning the name '"+name+"' is already defined, at '"+M.line+"' line tip: ->"+ M.message);
				return true;
			}
		...
		'log':function(M){
			...
			M = isObject(M) ? this.merge({},[M,defaults]) : defaults;
			throw new Error("Warning: at '"+M.line+"' line tip: ->"+ M.message);
		},
...
window[LHH_NAMESPACE_20150715_].main([window,LHH_CONFIG_20150717_],function(W,Config){
	 ...
	 *
	 */
	Basis.printTag=function(tag,single,D,content){
		tag = tag || null;
		single = single || false;
		...
		for(key in D){
			...
			values.push(' '+key+'="'+D[key]+'"');
		}
		if(single){
			label=['\<',tag,values.join(''),' \/\>'];
		}else{
			if(content){
				label=['\<',tag,values.join(''),'\>',content,'\<\/',tag,'\>'];
			}else{
				label=['\<',tag,values.join(''),'\>','\<\/',tag,'\>'];
			}
	...
	Basis.extends=function(superClass,type,arg) {
		...
		if(System && superClass ) {
			...
			throw new Error("Warning nothing the "+superClass);
			return 0;
		}
		/*------------------------------*/
