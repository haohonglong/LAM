...
window[LHH_NAMESPACE_20150715_].main([window,LHH_CONFIG_20150717_],function(W,Config){
	 ...
	 * @returns {String}
	 */
	System.templat=function(S){

		var ss=S.split('/'),vars,root,a=[],v=[];
		ss.each(function(i){
			if(i!=0){
				a.push(this);
			}
		...
		//没找到模版分隔符就返回传入的字符串
		if(-1 === ss[0].indexOf(System.Config.templat.leftLimit)){
			return S;
		}


		vars=ss[0].replace(System.Config.templat.leftLimit,'').replace(System.Config.templat.rightLimit,'');
		root =eval(vars);
	 ...
	 * @returns {String}
	 */
	System.replaceTpl=function(selector,attr_name){
		var value;
		$(selector).each(function(){
			value=System.templat($(this).attr(attr_name));
			console.log(value);
