...
window[LHH_NAMESPACE_20150715_].main([window],function(window,undefined){
	'use strict';
	var System=this;
	System.is(System,'Browser','EditTables');
	var __this__=null;

//设置多个表格可编辑
	function EditTables(D) {
		//无构造参数的方法
		...
		init.table.each(function(){
			SetTableCanEdit(this);
		});
...
//为单元格创建可编辑输入框
	function CreateTextBox(element, value) {
		...
		if (editState != "true") {
			...
			textBox.type = "text";
			textBox.EditTables = "EditCell_TextBox";
...
//为单元格创建选择框
	function CreateDropDownList(element, value) {
		...
		if (editState != "true") {
			...
			downList.EditTables = "EditCell_DropDownList";
...
//检查当前数据行中需要运行的字段
	function CheckExpression(row,expn) {
		System.each(row.cells,function(j){
			...
			if (expn) {
				...
				if (format) {
					//如指定了格式，进行字值格式化
					this.innerHTML = formatNumber(Expression(row, expn), format);
				} else {
					this.innerHTML = Expression(row, expn);
				}
			}
		});
...
///////////////////////////////////////////////////////////////////////////////////
	...
	//添加行
	EditTables.AddRow=function(table, index) {
...
//删除行
	EditTables.DeleteRow=function(table, index) {
	...
	//提取表格的值,JSON格式
	EditTables.GetTableData=function(table) {
	...
	System.extends(EditTables,System.Browser,1);
